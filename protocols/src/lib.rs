...
impl MediaProtocol for ImageProtocol {
    ...
    fn get_hash(&self) -> Sha256 { self.hash }
}

impl ImageProtocol {
    pub fn verify_image(&self, data: Vec<u8>) { assert_eq!(Sha256Digest::digest(&data).as_slice(), self.hash.0.as_slice()); }
    pub fn stream_image(&self, _file: impl AsyncRead + Unpin) -> impl Stream<Item = ImageBuffer<Rgba<u8>, Vec<u8>>> {
...
impl AudioProtocol {
    ...
    pub fn stream_audio(&self, _file: impl AsyncRead + Unpin) -> impl Stream<Item = AudioBuffer<f32>> {
